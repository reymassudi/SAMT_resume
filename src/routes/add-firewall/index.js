...
function AddFirewall({ showSnackbar, setCommit }) {
  ...
  const checkInputs = () => {
    ...
    } else if (!validateIP(values.ipaddr)) {
  ...
  const addFirewall = () => {
    ...
    postFormData.append('ipaddr', values.ipaddr);

    api.post("smt/fwadd.ms", postFormData)
      .then((response) => {
        setLoading(false);
        // setCommit(response.data.dirty);
        if (response.data.status === "success") {
          showSnackbar("فایروال با موفقیت ایجاد شد!", "success");
          setTimeout(
            () => window.location.href = "/firewall-list",
            500
          );
        } else {
          showSnackbar("خطا در ایجاد فایروال!", "error");
        }
      })
      .catch((error) => {
        setLoading(false);
            ...
            <Card>
              <MDBox py={3} px={5}>
                <Grid container spacing={2} justifyContent="center" sx={{ mt: 1, mb: 2 }}
                  ...
                  <Grid item md={6}>
                    <TextField id="ipaddr" label="IP*" variant="outlined" fullWidth
                               value={values.ipaddr}
                               onChange={(e) => handleChange(e.target.value, "ipaddr")} />
                  </Grid>
...
export default connect(null, mapDispatchToProps)(AddFirewall);
